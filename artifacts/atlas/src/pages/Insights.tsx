    ...
    <div className="w-full">
      ...
      <section className="relative min-h-[60vh] flex items-end bg-[#080E1C] pb-16 pt-40 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img src="https://images.unsplash.com/photo-1488229297570-58520851e868?auto=format&fit=crop&w=1920&q=80"
            alt="Strategic insights" className="w-full h-full object-cover opacity-20" />
              ...
              {insights && insights.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
                  ...
                  <Link href={`/insights/${insights[0].id}`} className="group block">
                    <div className="border border-border hover:border-[#00C4D4]/40 rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-black/8 transition-all duration-300">
                      <div className="grid grid-cols-1 lg:grid-cols-2">
                        {/* Image */}
                        <div className="relative h-64 lg:h-80 overflow-hidden">
                          <img src={INSIGHT_IMAGES[0]} alt={lang === "fr" ? (insights[0].titleFr ?? "") : (insights[0].titleEn ?? "")}
                            className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                          <div className="absolute inset-0 bg-gradient-to-r from-transparent to-[#080E1C]/20" />
                        </div>
                        {/* Content */}
                        <div className="p-8 md:p-10 flex flex-col justify-center">
                          ...
                          <div className="flex items-center gap-4 text-xs text-muted-foreground mb-6">
                            {insights[0].readTimeMinutes && (
                              <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" />{insights[0].readTimeMinutes} min</span>
                            )}
                  ...
                  {insights.slice(1).map((insight, i) => (
                    <motion.div key={insight.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                      ...
                      <Link href={`/insights/${insight.id}`} className="group block h-full">
                        <div className="h-full border border-border hover:border-[#00C4D4]/40 rounded-2xl overflow-hidden hover:shadow-lg hover:shadow-black/5 transition-all duration-300 flex flex-col">
                          {/* Thumbnail */}
                          <div className="relative h-48 overflow-hidden">
                            <img src={INSIGHT_IMAGES[(i + 1) % INSIGHT_IMAGES.length]} alt={lang === "fr" ? (insight.titleFr ?? "") : (insight.titleEn ?? "")}
                              className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                            <div className="absolute inset-0 bg-[#080E1C]/30" />
                            {insight.category && (
                          ...
                          <div className="p-6 flex flex-col flex-1">
                            ...
                            <div className="flex items-center justify-between text-xs text-muted-foreground">
                              {insight.readTimeMinutes && (
                                <span className="flex items-center gap-1.5"><Clock className="w-3 h-3" />{insight.readTimeMinutes} min</span>
                              )}
      ...
      {/* CTA */}
      <section className="relative py-20 bg-[#080E1C] overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img src="https://images.unsplash.com/photo-1573167507387-6b4b98cb7c13?auto=format&fit=crop&w=1920&q=80"
            alt="Partnership" className="w-full h-full object-cover opacity-10" />
        ...
        <div className="container mx-auto px-6 lg:px-8 relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            <h2 className="font-display text-3xl md:text-4xl text-white mb-3">{t("Restons connectés.", "Let's stay connected.")}</h2>
            <p className="text-white/50 max-w-lg">{t("Abonnez-vous à nos analyses stratégiques sur les corridors Canada-Afrique.", "Subscribe to our strategic analyses on Canada-Africa corridors.")}</p>
          </div>
          <Link href="/contact"
