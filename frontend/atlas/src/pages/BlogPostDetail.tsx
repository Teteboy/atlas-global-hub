...
export default function BlogPostDetail() {
  ...
  if (!post) {
    return (
      <div className="py-24 text-center">
        <h1 className="text-2xl font-bold text-gray-900 mb-4">{t("Article introuvable.", "Post not found.")}</h1>
        <Link href="/blog" className="text-[var(--color-atlas-primary)] hover:underline">
  ...
  const summary = lang === "fr" ? post.summaryFr : post.summaryEn;
  const body = lang === "fr" ? post.bodyFr : post.bodyEn;
  const published = new Date(post.publishedAt).toLocaleDateString(lang === "fr" ? "fr-FR" : "en-US", {
    year: "numeric",
    month: "long",
    day: "numeric",
  });
  ...
  return (
    ...
    >
      <section className="relative h-[45vh] min-h-[300px] flex items-end overflow-hidden">
        {post.imageUrl ? (
          <>
            <img src={post.imageUrl} alt={title} className="absolute inset-0 w-full h-full object-cover" />
